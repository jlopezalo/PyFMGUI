def get_params(params, method):
    ...
    if param_dict['contact_model'] == "paraboloid":
        param_dict['tip_param'] = analysis_params.child('Tip Radius').value() / 1e9 
    ...
    if method  in ("HertzFit", "Microrheo", "MicrorheoSine"):
        ...
        param_dict['max_ind'] = hertz_params.child('Max Indentation').value() / 1e9  
        param_dict['min_ind'] = hertz_params.child('Min Indentation').value() / 1e9 
        param_dict['max_force'] = hertz_params.child('Max Force').value() / 1e9 
        param_dict['min_force'] = hertz_params.child('Min Force').value() / 1e9 
        ...
        param_dict['rampspeed'] = ting_params.child('Ramp Speed').value() / 1e6
        ...
        param_dict['f0'] = ting_params.child('Init f0').value()
        param_dict['vdrag'] = ting_params.child('Viscous Drag').value()
        ...
        param_dict['contact_offset'] = ting_params.child('Contact Offset').value() / 1e6
